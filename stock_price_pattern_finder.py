...
class StockPatternFinderApp:
    ...
    def create_widgets(self):
        ...
        ttk.Label(frame, text="Enter Stock Symbol:").grid(row=0, column=0, sticky=tk.W)
        self.symbol_var = tk.StringVar()
        self.symbol_entry = ttk.Entry(frame, textvariable=self.symbol_var)
        self.symbol_entry.grid(row=0, column=1, sticky="we")

        ttk.Label(frame, text="Enter Period (e.g., 1mo, 3mo, 1y):").grid(row=1, column=0, sticky=tk.W)
        self.period_var = tk.StringVar(value="1mo")
        self.period_entry = ttk.Entry(frame, textvariable=self.period_var)
        self.period_entry.grid(row=1, column=1, sticky="we")

        ttk.Label(frame, text="Enter Interval (e.g., 1d, 1h):").grid(row=2, column=0, sticky=tk.W)
        self.interval_var = tk.StringVar(value="1d")
        self.interval_entry = ttk.Entry(frame, textvariable=self.interval_var)
        self.interval_entry.grid(row=2, column=1, sticky="we")

        self.fetch_button = ttk.Button(frame, text="Fetch Data & Find Patterns", command=self.fetch_and_find_patterns)
